...
from app.constants import BASE_LOG_CLOC, BASE_PROJECTS, BASE_LOG_CO_EVOLUTION, BASE_SUMMARY_MAINTENANCE_ACTIVITIES, \
    ...
    BASE_LOG_MAINTENANCE_ACTIVITIES, BASE_LOG_LOC, BASE_PROJECTS_FOLDER_NAME, IMPORTED_PROJECTS_FOLDER_NAME, \
...
from app.helpers.utils import save_json, get_json, is_valid_date, is_zero_or_one, get_comma_separated_names, parse_int
...
def save_code_distribution_details():
    ...
    for index, row in df.iterrows():
        ...
        if project_id in VALID_REPOS:
            ...
            if db_repository:
                ...
                uses_external_id = db_additional_data.uses_external_id
                ...
                if crud.exits_code_distribution_details_by_pipeline(db, db_pipeline.id):
                    logger.info(f"CodeDistributionDetail already exist for project_id {project_id} and pipeline.")
                    continue

                competences = crud.get_competences_by_pipeline(db, pipeline_id)
                revisions = [revision.competence for revision in competences]

                revision_length = len(revisions)
                commit_selected = range(revision_length)

                test_datas = crud.get_test_datas_by_pipeline(db, pipeline_id)
                ...
                for commit_order in commit_selected:
                    ...
                    if classify_test_based_on_function:
                        t_files = [data.file_path.replace('\\', '/').replace('//', '/') for data in filtered_test_datas
                                   if data.has_test_call == 1]

                    else:
                        t_files = [data.file_path.replace('\\', '/').replace('//', '/') for data in filtered_test_datas
                                   if data.is_test_file == 1]

                    valid_cloc = False

                    cloc_path_log = os.path.join(BASE_LOG_LOC, str(project_result.id), str(commit_order), 'cloc.log')

                    opened_file = open(cloc_path_log, "r", encoding="latin-1")
